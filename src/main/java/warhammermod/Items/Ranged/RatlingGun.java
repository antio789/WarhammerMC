...
public class RatlingGun extends AutogunBase {
    ...
    public void PlayaSound(World worldIn, PlayerEntity playerIn){
        worldIn.playSound(playerIn.getPosition().getX(),playerIn.getPosition().getY(),playerIn.getPosition().getZ(), SoundEvents.ENTITY_GENERIC_EXPLODE, SoundCategory.PLAYERS,0.6F,1.55F / (rand.nextFloat() * 0.4F + 1.2F) +  0.5F,true);
