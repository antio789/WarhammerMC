...
public class Slingtemplate extends BowItem {
    ...
    public Slingtemplate(Properties properties){
        super(properties);
            ...
            if (!itemstack.isEmpty() || consumeammo) {
                ...
                if ((double)f > 0.1D) {
                    ...
                    if (!worldIn.isRemote) {
                        ...
                        stone.shoot( playerentity.rotationPitch, playerentity.rotationYaw, 0.0F, f * 2.75F, 1.5F);
                    ...
                    worldIn.playSound((PlayerEntity)null, playerentity.getPosX(), playerentity.getPosYEye(), playerentity.getPosZ(), SoundEvents.ENTITY_ARROW_SHOOT, SoundCategory.PLAYERS, 1.0F, 1.0F / (random.nextFloat() * 0.4F + 1.2F) + f * 0.5F);
                    if (!consumeammo) {
                        itemstack.shrink(1);
                        if (itemstack.isEmpty()) {
