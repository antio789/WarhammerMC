...
public class DrakeGunTemplate extends AutogunBase {
    ...
    public DrakeGunTemplate(Item.Properties properties, int MagSize, int time) {
    ...
    public void fire(PlayerEntity player, World world, ItemStack stack){
        world.playSound(null, player.getPosition().getX(), player.getPosition().getY()-0.26, player.getPosition().getZ(), WarhammermodRegistry.flame, SoundCategory.PLAYERS, 0.3F, 1.05F+(rand.nextFloat()+rand.nextFloat())*0.1F);
        ...
        if(!world.isRemote){
            ...
            smallfireballentity.setPosition(player.getPosX(), (int)(player.getPosition().getY() + (double)(player.getHeight() - 0.35F)), player.getPosZ());
