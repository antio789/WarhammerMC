   ...
   ;
    ...
    private void spawnColoredParticles()
    {
        int i = 65280;
            ...
            for (int j = 0; j < 5; ++j)
            {
                this.world.addParticle(ParticleTypes.ENTITY_EFFECT, this.getPosition().getX() + (this.rand.nextDouble() - 0.5D) * (double)this.getWidth(), this.getPosition().getY() + this.rand.nextDouble() * (double)this.getHeight(), this.getPosition().getZ() + (this.rand.nextDouble() - 0.5D) * (double)this.getWidth(), d0, d1, d2);
            }
    }
