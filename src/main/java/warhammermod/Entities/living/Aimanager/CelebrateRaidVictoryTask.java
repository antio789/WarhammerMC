...
public class CelebrateRaidVictoryTask extends Task<DwarfEntity> {
   @Nullable
   private Raid raid;
   ...
   protected void updateTask(ServerWorld worldIn, DwarfEntity owner, long gameTime) {
      ...
      if (random.nextInt(200) == 0 && MoveToSkylightTask.func_226306_a_(worldIn, owner, new BlockPos(owner))) {
         ...
         FireworkRocketEntity fireworkrocketentity = new FireworkRocketEntity(owner.world, owner.getPosX(), owner.getPosY(), owner.getPosZ(), itemstack);
