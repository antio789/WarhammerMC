...
public class CreateBabyVillagerTask extends Task<DwarfEntity> {
   ...
   public CreateBabyVillagerTask() {
      super(ImmutableMap.of(WarhammermodRegistry.BREED_TARGET, MemoryModuleStatus.VALUE_PRESENT, MemoryModuleType.VISIBLE_MOBS, MemoryModuleStatus.VALUE_PRESENT), 350, 350);
   }
   ...
   protected void startExecuting(ServerWorld worldIn, DwarfEntity entityIn, long gameTimeIn) {
      DwarfEntity villagerentity = this.getBreedTarget(entityIn);
   ...
   protected void updateTask(ServerWorld worldIn, DwarfEntity owner, long gameTime) {
      DwarfEntity villagerentity = this.getBreedTarget(owner);
   ...
   protected void resetTask(ServerWorld worldIn, DwarfEntity entityIn, long gameTimeIn) {
      entityIn.getBrain().removeMemory(WarhammermodRegistry.BREED_TARGET);
   }

   private DwarfEntity getBreedTarget(DwarfEntity p_220482_1_) {
      return p_220482_1_.getBrain().getMemory(WarhammermodRegistry.BREED_TARGET).get();
   }

   private boolean func_220478_b(DwarfEntity p_220478_1_) {
      Brain<DwarfEntity> brain = p_220478_1_.getBrain();
      if (!brain.getMemory(WarhammermodRegistry.BREED_TARGET).isPresent()) {
         return false;
      } else {
         DwarfEntity villagerentity = this.getBreedTarget(p_220478_1_);
         return BrainUtil.isCorrectVisibleType(brain, WarhammermodRegistry.BREED_TARGET, EntityType.VILLAGER) && p_220478_1_.canBreed() && villagerentity.canBreed();
      }
   }
