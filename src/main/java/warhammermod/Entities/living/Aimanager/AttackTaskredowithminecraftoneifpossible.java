...
public class AttackTaskredowithminecraftoneifpossible extends Task<MobEntity> {
    @Nullable
    private Path path;
    @Nullable
    ...
    public AttackTaskredowithminecraftoneifpossible(int p_i50356_1_) {
    ...
    protected boolean shouldExecute(ServerWorld worldIn, MobEntity owner) {
        speed = owner.getAttribute(Attributes.field_233821_d_).getValue()*1.5F;
    ...
    private boolean canMoveto(MobEntity entity, WalkTarget target, long gametime) {
        ...
        if (!this.hasReachedTarget(entity, target)) {
            ...
            Vector3d vec3d = RandomPositionGenerator.findRandomTargetBlockTowards((CreatureEntity)entity, 10, 7, Vector3d.func_237492_c_(blockpos));
            if (vec3d != null) {
    ...
    private boolean hasReachedTarget(MobEntity p_220486_1_, WalkTarget p_220486_2_) {
        return p_220486_2_.getTarget().getBlockPos().manhattanDistance(new BlockPos(p_220486_1_.func_233580_cy_())) <= p_220486_2_.getDistance();
    }

    protected void checkAndPerformAttack(LivingEntity enemy, double distToEnemySqr, LivingEntity attacker) {
        double d0 = this.getAttackReachSqr(enemy,attacker);
    ...
    protected void alertOthers(MobEntity dwarf) {
        ...
        AxisAlignedBB axisalignedbb = AxisAlignedBB.func_241549_a_(dwarf.getPositionVec()).grow(d0, 10.0D, d0);
        List<MobEntity> list = dwarf.world.getLoadedEntitiesWithinAABB(dwarf.getClass(), axisalignedbb);
        ...
        protected double getTargetDistance(LivingEntity dwarf) {
            return dwarf.getAttribute(Attributes.field_233819_b_).getValue();
        }
    }
